...
module.exports = async function runSmokeSuite({ app, mainWindow, fs, path, probeMediaRange }) {
  ...
  const results = { formats: {}, operations: {} }
  ...
  const nativeOut = await until(`(() => { const s=window.__cutflowTest.state(); return s.marks.out>1.2 && s.marks.out<1.5 && !s.video.seeking ? s : null })()`, 5000, '出点手柄鼠标拖动')
  ...
  const playingState = await until(`(() => { const s=window.__cutflowTest.state(); return s.playing && s.video.currentTime>=.5 && s.video.currentTime<${nativeOut.marks.out} ? s : null })()`, 5000, '区间播放开始')
  ...
  results.operations = {
    ...
    intervalStartedAtIn: playingState.video.currentTime >= 0.43,
  ...
  if (!results.operations.stableMount || !results.operations.stableSourceAfterSeekAndTransform || !results.operations.seekTargetHeld || !results.operations.nativeRangeDrag || !results.operations.nativeInHandleDrag || !results.operations.nativeOutHandleDrag || !results.operations.stoppedAtOut || !results.operations.noReloadOnTransform || !results.operations.waveformClickSeek || !results.operations.waveformWheelZoom || !results.operations.range206 || !results.operations.deleteClip || !results.operations.reorderClips) {
